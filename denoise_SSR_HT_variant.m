function [s_noise_hard, s_SSR_HT_var, h,Lh] = denoise_SSR_HT_variant(...
    s_noise, Nr, clwin, Nfft, window, sigma_opt, cas)
...
%     h = amgauss(2*Lh+1,Lh+1,L); 
    [h, Lh] = create_gaussian_window(length(s_noise), Nfft, sigma_opt);
end

[tfr_noise] = tfrstft(s_noise,Nfft,cas,h,Lh);
...
s_noise_hard = zeros(length(s_noise), Nr);
% s_SSR_HT_init = zeros(size(sn), nr);
s_SSR_HT_var = zeros(length(s_noise), Nr);
...
%             tfr_noise_soft_init(:,r) = YY' + 1i*ZZ';
    ...
    s_noise_hard(:, j) = itfrstft(tfr_noise_hard,cas,h);
%     s_SSR_HT_init(:, j) = itfrstft(tfr_noise_soft_init,cas,h);
    
    cmode = itfrstft(tfr_noise_soft_var,cas,h);
    s_SSR_HT_var(:, j) = cmode;
end
